(function(){
  ...
  function buildTree(posts){
    ...
    (posts||[]).forEach(p => {
      const parts = normalizePath(p.path).split('/'); // keep 'posts' top-level
      let node = root;
  ...
  function addDailyNav(posts, entry){
    ...
    nav.innerHTML = `
      <div>${prev ? `<a class="prev" href="${toRootHref(prev.path)}">← ${prev.date || prev._d}</a>` : ''}</div>
      <div>${next ? `<a class="next" href="${toRootHref(next.path)}">${next.date || next._d} →</a>` : ''}</div>
    `;

    const meta = $('#post-meta', article);
  ...
  function enhancePostPage(posts){
    ...
    metaWrap.innerHTML='';
    const dateText=entry?.date||dateFromMeta(); if(dateText){ const d=document.createElement('span'); d.className='muted'; d.textContent=new Date(dateText).toLocaleDateString(); metaWrap.appendChild(d); }
    const tags=entry?.tags||tagsFromMeta(); tags.forEach(t=>metaWrap.appendChild(chip(t)));

    if(!$('a.backlink', article)){ const back=document.createElement('a'); back.href=toRootHref('index.html'); back.className='backlink'; back.textContent='← Back to Home'; article.appendChild(back); }
